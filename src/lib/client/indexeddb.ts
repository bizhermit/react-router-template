...
type StoreNames<S> = Exclude<S, number | symbol> | Array<Exclude<S, number | symbol>>;
...
export interface IndexedDBController<S extends AnyStores> {
  db: IDBDatabase;
  read<Store extends keyof S, R>(
    parmas: {
      storeNames: StoreNames<Store>;
      options?: IDBTransactionOptions;
    },
    process: (
      stores: { [K in Store]: IndexedDBStoreReadController<S[K]>; },
      controllers: SubController,
    ) => Promise<R>
  ): Promise<R>;
  write<Store extends keyof S, R>(
    parmas: {
      storeNames: StoreNames<Store>;
      options?: IDBTransactionOptions;
    },
    process: (
      stores: { [K in Store]: IndexedDBStoreWritableController<S[K]>; },
      controllers: SubController,
    ) => Promise<R>
  ): Promise<R>;
};
  ...
  return await new Promise<IndexedDBController<T>>((resolve, reject) => {
    ...
    function ready(db: IDBDatabase) {
      ...
      async function transaction<R>(
        ...
        process: (stores: any, subcon: SubController) => Promise<R>,
      ) {
        ...
        (typeof params.storeNames === "string" ? [params.storeNames] : params.storeNames).forEach(storeName => {
        ...
        const ret = await process(stores, {
          abort: trans.abort,
        });
