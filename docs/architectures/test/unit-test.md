...
> **運用優先度**: 本書は人間レビューにも対応するが、AI エージェントによる自動実行手順を優先する。AI は本書の「AI エージェント向け作業手順」と「実行結果レポート（必須フォーマット）」を満たすこと。

## クイックスタート

- **Step1: 対象と責務を確認** — 公開インターフェースと副作用を列挙する。
- **Step2: テスト設計** — 下記チェックリストを使い正常/異常/副作用/境界ケースを決める。
- **Step3: テスト骨子作成** — 準備（Arrange）/実行（Act）/検証（Assert）をコメントで先に書き、実装を流し込む。
...
> - [ ] 非決定要素は全て制御している
> - [ ] UIを含むテストでロール・ラベル・キーボード操作の観点を確認している
...
- 可読性優先: 準備（Arrange）/実行（Act）/検証（Assert）を明示し、テスト名で意図を伝える。
- 最小限の準備: フィクスチャやヘルパーは重複排除のために小さく保ち、過度な抽象化は避ける。

## 変更種別ごとの必須テスト範囲

- UI文言・スタイルのみの変更: 影響コンポーネントのユニットテストを更新し、必要に応じてコンポーネントテストを追加する
- UIの入力/操作仕様変更: ユニットテストに加えて、キーボード操作・ラベル・ARIA属性の観点を含むテストを追加する
- ドメインロジック変更: 当該ロジックのユニットテストを必須とし、重要分岐・境界値・異常系を追加する
- API契約・永続化・認証認可に関わる変更: ユニットテストに加えて、[インテグレーションテスト](./integration-test.md)で契約整合を確認する
- 画面遷移をまたぐユーザーフロー変更: ユニットテストに加えて、必要に応じて[コンポーネントテスト](./component-test.md)または[E2Eテスト](./e2e-test.md)を追加する
...
- ファイル名: `<対象ファイル名>.unit.test.ts(x)` を基本とする（互換のため既存 `<対象ファイル名>.test.ts(x)` も当面許容）。
...
- モックの後始末として、`afterEach`で`vi.restoreAllMocks()`または`vi.unstubAllGlobals()`を実行し、テスト間汚染を防ぐ。
...
- 主要な操作要素が`getByRole`と表示名で取得できること
- フォーム入力が`label`または`aria-label`/`aria-labelledby`で識別できること
- キーボード操作（Tab移動、Enter/Space）で主要操作が実行できること
- 状態変化（ローディング、エラー、無効）が`aria-*`属性または`disabled`で表現されていること
...
- テスト配置想定: `src/lib/shared/timing/_tests/debounce.unit.test.ts`（未作成の場合は新規作成する）
...
- テスト配置想定: `src/lib/shared/fetch/_tests/core.unit.test.ts`（未作成の場合は新規作成する）
...
describe("generateApiAccessor", () => {
	...
	it("GET で path/query/header を構築する", async () => {
		...
		expect(fetchMock).toHaveBeenCalledWith("/v1/users/u1?q=a", {
			method: "GET",
...
4. 準備（Arrange）/実行（Act）/検証（Assert）の順でテストを書く。可読性を優先し、不要なヘルパーを作らない。
5. 実装変更に強いテストかを自己レビューし、冗長な共有化や抽象化を避ける。
6. `npm run test` または対象を絞る場合は `npx vitest run <targetFilePath>` を実行し、失敗内容を解消してからレビューへ回す。

### 完了条件（AI 自動実行）

- 対象モジュールの主要ケース（正常/異常/境界）がテストでカバーされている。
- 非決定要素（時間・乱数・外部依存）が制御されている。
...
- 非決定要素が排除されているか。
- UIを含む場合、ロール・ラベル・キーボード操作・ARIA属性の観点を満たしているか。
- 不要なモックや過剰な抽象化がないか。
- セキュリティ観点（ログに秘密情報が出ないか等）が担保されているか。

## レビュー提出物

- 「実行結果レポート（必須フォーマット）」の 4 項目を提出する。
- 追加で共有が必要な事項（判断理由、代替案、保留事項）があれば補足として記載する。

## 参考

- 他ドキュメントとの関係は [テスト仕様](./index.md) を参照。
