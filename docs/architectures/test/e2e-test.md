...
- `playwright/screenshot.spec.ts` は、画面定義書・設計資料に貼り付ける注入画像を生成するための補助テストとして扱う。
...
> 補足: `playwright/screenshot.spec.ts` は資料向け画像生成用のため、本ケーススタディの配置先には使用しない。
...
5. 失敗系を含む最低シナリオ本数を満たしていることを確認する（本書「完了判定基準（必須）」参照）。
6. 終了後はデータクリーニングを実行し、再実行可能な状態を維持する。
