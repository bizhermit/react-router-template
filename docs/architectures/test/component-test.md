...
> **命名規約**: テスト種別サフィックスは `unit / int / comp` を採用する。コンポーネントテストは `*.comp.test.ts(x)` とする（互換のため既存 `*.component.test.ts(x)` も当面許容）。
...
## ケーススタディ: Button / LinkButton コンポーネント

- 対象: [src/lib/components/elements/button/button.tsx](../../../src/lib/components/elements/button/button.tsx)
- 対象: [src/lib/components/elements/button/link-button.tsx](../../../src/lib/components/elements/button/link-button.tsx)
- テスト: [src/lib/components/elements/button/_tests/button.comp.test.tsx](../../../src/lib/components/elements/button/_tests/button.comp.test.tsx)
- テスト: [src/lib/components/elements/button/_tests/link-button.comp.test.tsx](../../../src/lib/components/elements/button/_tests/link-button.comp.test.tsx)

| シナリオ                      | 前提/操作                                           | 期待する結果                                            |
| ----------------------------- | --------------------------------------------------- | ------------------------------------------------------- |
| Button の有効状態クリック     | `disabled=false` でレンダリングし、クリック         | `onClick` が 1 回呼ばれる                               |
| Button の多重クリック防止     | 非同期 `onClick` を設定し、短時間で 2 回クリック    | `onClick` が 1 回だけ呼ばれ、ボタンが `disabled` になる |
| LinkButton の通常遷移         | `MemoryRouter` 上で `to="/next"` を指定してクリック | `location.pathname` が `/next` に更新される             |
| LinkButton の無効状態         | `disabled=true` でクリック                          | 遷移せず `location.pathname` が変わらない               |
| LinkButton のアクセシビリティ | `disabled=true` でレンダリング                      | `role="button"` と `aria-disabled="true"` が出力される  |
