...
- すべての変更は[docs/architectures/security-checklist.md](docs/architectures/index.md)を参照し、該当項目を網羅的に確認する
- 不明点がある場合は実装を進めず、状況と懸念点をユーザーへ必ず共有する

### インプット／インジェクション対策

- SQL/NoSQL/コマンドいずれも文字列連結でクエリを生成せず、必ずプレースホルダー・パラメータバインディングを使用する
- シリアライズされるユーザー入力は型・フォーマット・長さを検証し、サニタイズ不可能なデータは即座に拒否する
- JSON やテンプレート出力ではエスケープ処理を徹底し、危険な文字列をロギングする際はマスクする

### 認証・認可

- `features/auth` に依存する機能の改修時は、想定フロー（登録・認証・トークン更新・サインアウト）が破綻しないかを順番に検証する
- 認可判定はクライアント側だけに依存せず、必ずサーバー側でロール・スコープを再検証する
- セッション／トークン／クッキーを取り扱う箇所では、有効期限・失効処理・再利用防止（nonce, state 等）の確認をコードレビュー対象に含める

### ログ・監査・構成

- ログ・エラーメッセージに認証情報や個人情報が出力されないよう必ず確認し、必要に応じてモックテストで検証する
- 新規依存関係や設定変更は、既存の CSP・ヘッダー設定・ACL に影響しないかを `docs/architectures/security-checklist.md` の該当節で確認する
- リファクタやドキュメント更新であっても、セキュリティ観点の差分を自己レビュー項目に含め、必要なら追加テスト（例: 認証ガードのユニットテスト）を作成する
